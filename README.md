...
| **Pillar C2: Byzantine-Robust Fairness Aggregation** | Server | **BFWA** (Bi-objective Frank–Wolfe Aggregation) runs a penalised Frank–Wolfe iteration on the probability simplex $\Delta_K$ that *steers* aggregation weights toward an operator-chosen fairness budget $\tau$ on the **reported client** disparity; feasibility ($w \cdot \text{dpd} \le \tau$) is **not guaranteed** and is reported per round as the `constraint_residual` / `feasible` fields of `aggregate()`. **Coordinate-wise Median** admits no client-weight vector and consumes no self-reported metadata, so metadata deception cannot influence it by construction; its robustness is bounded by the standard $f < K/2$ breakdown point, not by an empirical immunity result. |
