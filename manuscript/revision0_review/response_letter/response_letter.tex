...
Following the Editor's instruction to check the consistency between the text and
the reported tables, we re-verified \emph{every} numerical claim in the paper
against the table it cites. Text and tables now agree throughout.
...
Reviewer~1 anticipated, all changes concern wording, scope and consistency. One
figure and two table columns were regenerated from the \emph{existing} logged
results, with no re-training; we note each below.
...
Editor/R1 & Overstated claims & \S5.8 p.~25, \S5.3 p.~18, \S6 p.~31 \\
Editor/R1 & Text/table consistency & \S5.3, \S5.4, \S5.6, \S5.8 \\
...
Editor/R2 & Trust-score limitations & \S6 p.~31 (moved before the score) \\
R1.1 & \texttt{robust\_bfwa} exclusivity claim & \S5.8 p.~25, \S5.4 p.~20 \\
R1.2 & Sixteen vs.\ fifteen baselines & Throughout; \S5.2 p.~15 \\
R1.3 & Uneven baseline coverage & New paragraph, \S5.2 p.~17 \\
...
R1.6 & Five text/table mismatches & \S5.3, \S5.4, \S5.6 \\
R1.7 & DP scope in the abstract & Abstract; Table~1 note \\
R2.1 & Inference-time $s$ needs a full treatment & New paragraph \S6 p.~32; Limitation (12) p.~36 \\
R2.2 & Trust-score caveat comes too late & \S6 p.~31; Limitation (10) \\
\bottomrule
\end{tabular}
...
Done \where{Declarations, p.~38}. The Declarations were previously a single
...
Done \where{Declarations, p.~38}. The manuscript now contains an explicit
...
We also strengthened the \textbf{Code availability} statement. The previous
wording (``available from the corresponding author upon request $\dots$ will be
released publicly upon acceptance'') sat awkwardly beside the paper's
reproducibility claims. The repository is now public and cited directly:
\url{https://github.com/vinhqdang/FedFairGNN}. It contains the source code, the
serialised configuration defining every reported run, and the generated tables
and figures; since every dataset is public and every run is fully specified by
its configuration, any reported number can be reproduced end to end from the
repository alone.
...
withdrawn and replaced with an accurate two-aggregator statement (\S5.8), and
the claim about non-private baselines on German AUC now names the exception
(\S5.3); both are detailed under R1.1 and R1.6(c) below. We also went through
the paper's remaining comparative statements and bounded each to what the
corresponding table supports.
\item \textbf{Text/table consistency.} Every numerical claim in the paper has
been re-verified against the table it cites; text and tables now agree
throughout.
...
passage now reads \where{\S5.8, p.~25}:
...
We have applied the same correction to the parallel statement in the ablation
discussion \where{\S5.4, p.~20}, so the two sections now agree. The revised
passage also names the remaining cells in which a competitor leads: trimmed mean
attains the lowest EOD under fairness poisoning ($0.005$) and multi-Krum the
lowest EOD under Gaussian corruption ($0.017$).
...
\S5.2, \S7 conclusion), removed FedFB from the list of \emph{evaluated}
baselines in \S5.2 and \S2, and retained it only in the related-work survey,
where it belongs as literature. To prevent the ambiguity recurring, \S5.2 now
ties the count directly to the tables \where{\S5.2, p.~15}:
...
the coverage exactly \where{\S5.2, p.~17}:
...
discussed: the Elliptic paragraph in \S5.3 now says explicitly that only two
...
evaluation'' \where{\S7, Limitation (4), p.~35}.
...
Neither option applies, for a reason worth stating precisely: our energy figure
is a \emph{proxy} computed as wall-clock $\times$ an assumed CPU draw, and
wall-clock was not recorded for those canonical small-graph runs, so the column
had no measurement behind it rather than one below resolution. Increasing the
precision would therefore not have produced a comparison. We have instead made
two changes that give the reviewer the comparison the comment is asking for:
...
canonical small-graph runs, so the proxy of Section~4.5 has no input for them.
...
The table is correct and the text now reads $0.021$ \where{\S5.3, p.~19}. (The
...
\textbf{(c) The AUC claim does not hold for FairGNN.} Correct: FairGNN's
$p{=}0.16$ is not significant. The sentence now names both the rule and the
exception \where{\S5.3, p.~18}:
...
\textbf{(d) The attribute-inference baseline: $0.50$ versus $0.53$.} The two
numbers refer to two different quantities, and the figure labelled the wrong one
as ``chance''. Our attack samples targets \emph{in equal numbers from each true
group} ($2{,}000$ per group), so an adversary that ignores the release entirely
cannot beat $0.500$ on that target set --- which is the baseline the table
reports. The figure's line at $0.53$ is the majority-group \emph{prevalence} in
Bail, which is the chance level only for an \emph{unbalanced} target set.

We have re-rendered Figure~3 from the already-logged attack results (no
...
it is. The text \where{\S5.6, p.~24} and both captions now explain the
distinction, and the plotting script was patched so the mislabelling cannot
return.

We kept $0.500$ as the reference because it is both the correct comparison for a
balanced target set and the more demanding one: it requires the mechanism to
reduce the adversary to a coin flip rather than to a class-prior guess. Against
that reference the result is stronger than we had previously stated. Each row of
Table~8 is a Monte-Carlo estimate over $4{,}000$ target draws, so its standard
error is $\sqrt{0.25/4000}\approx0.008$, and the largest excess over chance in
the entire sweep is $0.013$, or $1.6$ standard errors. We therefore added:
...
tables aggregate the full seed sets (10 on German, 5 on Bail). The ablation
caption now states this explicitly and shows that all four affected cells differ
by less than one standard deviation of the multi-seed mean
\where{Table~7, p.~23}.
...
We agree that this warrants a full treatment: it is the design decision in
TrustFedGNN with the largest practical and legal footprint for a deployer.
\S6 now
...
We agree, and have inverted the order so the caveat precedes the number rather
than following it. The subsection is retitled
``\textbf{Composite trust score: read this caveat first}'' and now opens with the
limitation, before the score is defined \where{\S6, p.~31}:
...
We believe the manuscript is considerably stronger for these comments. The
technical content is unchanged --- no result has been altered and no new
experiment was required --- but the claims are now precisely bounded by the
evidence, the text and the tables agree throughout, the scope of the privacy
guarantee is unambiguous, and the two governance questions the Editor and
Reviewer~2 raised (inference-time access to protected attributes, and the
standing of the composite trust score) are treated at the length they deserve.
