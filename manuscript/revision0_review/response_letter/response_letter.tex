...
While making that change we found, and corrected, an inaccuracy of our own in
the same statement. An earlier draft of this revision said the experiment logs
were ``included in the code repository''. They are not: the repository holds the
code, the serialised configuration that defines every reported run, and the
generated tables and figures, but the raw per-run log files are not committed.
Because every dataset is public and every run is fully specified by its
configuration, any reported number can still be reproduced end to end from the
repository alone --- but that is a different claim from shipping the logs, and
the manuscript now makes exactly the claim that is true, in all five places
where availability is described: Data availability, Code availability,
Supplementary information, the Reproducibility paragraph of \S7, and
Appendix~C. The raw logs are offered from the corresponding author on request.
We would rather correct this ourselves than have a reader discover that a stated
artifact is absent.
...
corrected to name the exception (\S5.3); the ``lowest DPD/EOD of any method''
claim for PUFFLE is corrected to a tie (\S5.3); and the Elliptic result is
restated to include the fact that EOD moves the \emph{wrong} way (\S5.3).
...
corrected in the same way \where{\S5.4, p.~20}. While re-reading Table~9 we
...
sentence now names both the rule and the exception \where{\S5.3, p.~18}:
...
\item \textbf{An overstated claim about PUFFLE.} \S5.3 said PUFFLE ``reaches the
...
\S5.3. It previously read that TrustFedGNN ``attains strong subgroup fairness at
