...
We also strengthened the **Code availability** statement. The previous wording ("available from the corresponding author upon request … will be released publicly upon acceptance") sat awkwardly beside the paper's reproducibility claims. The repository is now public and cited directly: https://github.com/vinhqdang/FedFairGNN — it contains the source code, the serialised configuration defining every reported run, and the generated tables and figures; since every dataset is public and every run is fully specified by its configuration, any reported number can be reproduced end to end from the repository alone.
