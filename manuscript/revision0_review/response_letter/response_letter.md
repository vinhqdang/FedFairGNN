...
| Editor | Declarations need separate subheadings | Declarations, p. 38 |
| Editor | Data availability statement | Declarations, p. 38 |
| Editor / R1 | Overstated claims | §5.8 p. 25, §5.3 p. 18, §6 p. 31 |
| Editor / R1 | Text/table consistency | §5.3, §5.4, §5.6, §5.8 |
...
| Editor / R2 | Trust-score limitations | §6 p. 31 (moved before the score) |
| R1.1 | `robust_bfwa` exclusivity claim | §5.8 p. 25, §5.4 p. 20 |
| R1.2 | Sixteen vs. fifteen baselines | Throughout; §5.2 p. 15 |
| R1.3 | Uneven baseline coverage | New paragraph, §5.2 p. 17 |
...
| R1.6 | Five text/table mismatches | §5.3, §5.4, §5.6 |
...
Done (Declarations, p. 38). The Declarations were previously a single bulleted list. They are now a section in which each item carries its own subheading: Funding; Conflict of interest; **Ethical approval**; **Consent to participate**; **Consent to publish**; Data availability; Materials availability; Code availability; Author contributions. Each is addressed substantively rather than with a bare "not applicable" — for example:
...
Done (Declarations, p. 38). The manuscript now contains an explicit **Data availability** subheading naming every dataset and its public source with URLs, and stating that no new data were generated. We will select "yes" in the submission system and paste this statement verbatim so the two match exactly.

We also strengthened the **Code availability** statement. The previous wording ("available from the corresponding author upon request … will be released publicly upon acceptance") sat awkwardly beside the paper's reproducibility claims. The repository is now public and cited directly: https://github.com/vinhqdang/FedFairGNN

While making that change we found, and corrected, an inaccuracy of our own in the same statement. An earlier draft of this revision said the experiment logs were "included in the code repository". They are not: the repository holds the code, the serialised configuration that defines every reported run, and the generated tables and figures, but the raw per-run log files are not committed. Because every dataset is public and every run is fully specified by its configuration, any reported number can still be reproduced end to end from the repository alone — but that is a different claim from shipping the logs, and the manuscript now makes exactly the claim that is true, in all five places where availability is described: Data availability, Code availability, Supplementary information, the Reproducibility paragraph of §7, and Appendix C. The raw logs are offered from the corresponding author on request. We would rather correct this ourselves than have a reader discover that a stated artifact is absent.
...
The reviewer is correct and we have withdrawn the claim. Trimmed mean retains 0.984/0.956/0.983 AUC across the three attacks against our 0.992/0.953/0.990, so it survives the suite as we do and is indeed marginally better under ALIE. The passage now reads (§5.8, p. 25):
...
We corrected the count to **fifteen** in all six places where it appeared (abstract, §1 contribution 5, §1 relation-to-preliminary-version, §5.2, §7 conclusion), removed FedFB from the list of *evaluated* baselines in §5.2 and §2, and retained it only in the related-work survey. To prevent recurrence, §5.2 now ties the count directly to the tables (p. 15):
...
Agreed — this was left to be inferred from dashes in the tables, which is not good enough. The experimental setup now carries a dedicated paragraph (§5.2, p. 17) stating that the full fifteen-baseline comparison runs only on German, Bail and Pokec; that Credit runs against the seven core baselines plus DP-FedAvg; that Elliptic runs against only FedAvg-GAT and FairSIN, the two baselines that scale within our CPU budget; and that a "--" always means *not run*, never a failed or omitted result. The consequence is propagated to the Elliptic discussion in §5.3 ("not a broad competitive ranking"), to the introduction, and to a new limitation item (4b).
...
The limitations section was rewritten to match, and now names the robustness study as the place where statistical power runs out and its multi-seed replication as "the most valuable single addition to this evaluation" (§7, Limitation (4), p. 35).
...
**(a) FedFACT DPD on Bail.** The text said 0.015; Table 5 says 0.021. The table is correct and the text now reads 0.021 (§5.3, p. 19). The surrounding claim — that FedFACT achieves the lowest DPD among non-private methods on Bail — remains true at the corrected value.
